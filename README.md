...
当前训练入口直接读取离线 `feature_database.npz`，不再在训练启动时从 `database.npz` 现场构建特征。
...
```text
feature_database: data/processed/100style_test5_pruned/feature_database.npz
epochs: 100
batch_size: 32
lr: 2e-4
min_lr: 1e-5
...
训练前先基于现有 `database.npz` 导出离线 `feature_database.npz`：

```bash
python preprocess/build_feature_database.py \
  --database data/processed/100style_test5_pruned/database.npz \
  --output data/processed/100style_test5_pruned/feature_database.npz
```

输出 `feature_database.npz` 包含：
