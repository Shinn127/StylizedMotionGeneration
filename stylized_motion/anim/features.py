...
#: The SOMA bind skeleton every somaview render and FK is skinned against.
SOMA_BIND_BVH = Path("data/assets/somaview/SOMA_bind.bvh")
#: The bind BVH is authored in centimetres; feature space is metres.
SOMA_BIND_UNIT_SCALE = 0.01


def bind_reference_positions(
    names: list[str] | tuple[str, ...],
    *,
    bind_bvh: str | Path | None = None,
    unit_scale: float = SOMA_BIND_UNIT_SCALE,
    mirror: bool = False,
) -> np.ndarray | None:
    """Local joint offsets in metres for a feature skeleton, from the bind contract.

    ``MotionFeatureStats.ref_pos`` is the *dataset mean* of the local positions,
    not the skeleton: the packed store is built from original **and mirrored**
    clips, mirroring flips the lateral axis, and every joint whose offset is
    constant and axis-aligned therefore averages to ~0 (``Spine1``, ``Spine2``,
    ``Chest``, ``Neck2``, ``Head`` in the SEED SOMA store).  Rebuilding a skeleton
    from that mean collapses the spine and hides joint motion, so FK, rendering
    and world-space metrics must take the offsets from the bind skeleton -- the
    contract the viewer skins the mesh against.

    ``mirror=True`` negates the lateral (x) component: the store's mirrored clips
    (``clip_mirror``, the ``_M`` variants) are reflected versions of the same
    skeleton, and the feature vector carries rotations but not chain offsets, so
    the *consumer* has to mirror the reference skeleton for them.  Posing a
    mirrored window on the unmirrored offsets folds the torso over (the spine
    chain's offsets are lateral).

    Returns ``None`` when the bind file (or a requested joint) is unavailable, so
    callers fall back to the store's values instead of inventing a skeleton.
    """
    path = Path(bind_bvh) if bind_bvh is not None else SOMA_BIND_BVH
    if not path.exists():
        return None
    from stylized_motion.anim.genoview import load_bvh_data

    data = load_bvh_data(path)
    bind_names = [str(name) for name in data["names"]]
    offsets = np.asarray(data["offsets"], dtype=np.float64) * float(unit_scale)
    lookup = {name: offsets[index] for index, name in enumerate(bind_names)}
    reference = np.zeros((len(names), 3), dtype=np.float32)
    matched = 0
    for index, name in enumerate(names):
        # 'Simulation' is generated by the feature pipeline and is absent from the
        # bind BVH; its position comes from the root channels, not from an offset.
        # The mirror is the partner joint's offset with x negated -- the same
        # L/R swap plus lateral flip the pipeline applies, not a blanket x-flip:
        # the limb chains keep their sign because their names already swapped.
        source_name = mirror_partner(str(name)) if mirror else str(name)
        offset = lookup.get(source_name)
        if offset is None:
            continue
        if mirror:
            offset = offset * np.array([-1.0, 1.0, 1.0], dtype=np.float64)
        reference[index] = offset.astype(np.float32)
        matched += 1
...
def mirror_partner(name: str) -> str:
    """``LeftArm`` -> ``RightArm`` (and back); other names map to themselves."""
...
def stats_with_reference_skeleton(
    stats: MotionFeatureStats,
    names: list[str] | tuple[str, ...],
    *,
    bind_bvh: str | Path | None = None,
    keep_moving_joints: bool = True,
    mirror: bool = False,
) -> MotionFeatureStats:
    """``stats`` with ``ref_pos`` taken from the bind skeleton (metres).

    The moving joints (``Simulation``, ``Hips``) keep their stored values: the
    root is integrated from its velocity and the hips position is a real feature,
    so only the constant offsets are replaced.  ``mirror=True`` for the store's
    mirrored clips (see :func:`bind_reference_positions`).
    """
    reference = bind_reference_positions(names, bind_bvh=bind_bvh, mirror=mirror)
    if reference is None:
        return stats
    resolved = reference.copy()
    if keep_moving_joints:
        for index, name in enumerate(names):
            if str(name) in {"Simulation", "Hips"}:
                resolved[index] = np.asarray(stats.ref_pos[index], dtype=np.float32)
...
def build_motion_feature_components(database: dict[str, np.ndarray]) -> MotionFeatureComponents:
    ...
    if x_contacts.ndim != 2 or x_contacts.shape[1] != 2:
        # The feature contract keeps two contact channels (left/right toe); a
        # per-joint contact array would silently build a vector of the wrong width.
        raise ValueError(
            f"contacts must be [T, 2] (left/right toe), got {x_contacts.shape}"
        )
