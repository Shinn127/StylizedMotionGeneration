...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("outputs/generator_flat_fsq_40x9/best.pt"),
    )
    parser.add_argument(
        "--token-database",
        type=Path,
        default=Path("data/processed/100style_pruned/flat_fsq_40x9"),
        help="Range metadata; token files are only used with --seed-source token-db.",
    )
    parser.add_argument(
        "--fsq-checkpoint",
        type=Path,
        default=Path("outputs/flat_fsq_40x9/best.pt"),
    )
...
def load_generator(path: Path, store, device: torch.device):
    ...
    if checkpoint.get("schema_version") != 2 or checkpoint.get("model_family") != "generator":
    ...
    if checkpoint.get("feature_schema") != store.feature_schema:
...
def load_fsq(path: Path, store, device: torch.device):
    ...
    checkpoint, model = load_representation_checkpoint(path, device, feature_schema=store.feature_schema)
...
def validate_tokenizer_contract(
    ...
    generator_tokenizer_sha256 = str(generator_checkpoint["token_store_checkpoint_sha256"])
    ...
    if token_store.checkpoint_sha256 != generator_tokenizer_sha256:
        raise ValueError(
            "TokenStore and generator checkpoint use different tokenizer checkpoints: "
            f"store={token_store.checkpoint_sha256}, generator={generator_tokenizer_sha256}"
        )
...
def encode_seed_from_feature_database(
    ...
    checkpoint_stats = fsq_checkpoint.get("feature_stats")
            ...
            json.dumps(
                {
                    ...
                    "token_store_checkpoint_sha256": str(generator_checkpoint["token_store_checkpoint_sha256"]),
