...
whereas downstream token workflows consume one FSQ token file per motion range.  This
...
the same shard layout as the token database.  It intentionally stores raw
  ...
  downstream conditioning workflow.
"""
...
def _source_range_metadata(data: np.lib.npyio.NpzFile, trajectory_path: Path) -> tuple[np.ndarray, ...]:
    ...
    if "database_path" not in data.files:
        raise ValueError(
            ...
            "stylized_motion.data.build_trajectory_inputs from the current repository."
        )
...
def build_trajectory_database(
    ...
    token_store = build_token_store(token_database)
    ...
    if "trajectory_feature_order" not in data.files:
        raise ValueError("Trajectory input must contain trajectory_feature_order")
    feature_order = str(np.asarray(data["trajectory_feature_order"]).item())
    np.savez(
        metadata_path,
        schema_version=np.asarray(2, dtype=np.int32),
        token_database=np.asarray(str(token_database), dtype=object),
        token_store_checkpoint_sha256=np.asarray(token_store.checkpoint_sha256, dtype=object),
        representation_family=np.asarray(token_store.representation_family, dtype=object),
        representation_variant=np.asarray(token_store.representation_variant, dtype=object),
        representation_id=np.asarray(token_store.representation_id, dtype=object),
        feature_schema_hash=np.asarray(token_store.feature_schema_hash, dtype=object),
        frame_rate=np.asarray(token_store.frame_rate, dtype=np.int32),
        temporal_downsample=np.asarray(token_store.temporal_downsample, dtype=np.int32),
        receptive_field=np.asarray(token_store.receptive_field, dtype=np.int32),
        context_left=np.asarray(token_store.context_left, dtype=np.int32),
        lookahead_frames=np.asarray(token_store.lookahead_frames, dtype=np.int32),
        decoder_passes_inference=np.asarray(token_store.decoder_passes_inference, dtype=np.int32),
