...
from .token_store import TokenDataset, TokenStore, TokenWindow
...
class TrajectoryNormalization:
    ...
    def from_checkpoint(cls, values: dict[str, object]) -> "TrajectoryNormalization":
        required = {"mean", "std", "valid_frames"}
        missing = sorted(required - set(values))
        if missing:
            raise ValueError(f"Trajectory normalization is missing required fields: {missing}")
        return cls(
            mean=np.asarray(values["mean"], dtype=np.float32),
            std=np.asarray(values["std"], dtype=np.float32),
            valid_frames=int(values["valid_frames"]),
        )
...
def build_trajectory_store(database: str | Path, token_store: TokenStore) -> TrajectoryStore:
    ...
    if int(np.asarray(data["schema_version"]).item()) != 2:
        raise ValueError("TrajectoryStore schema_version must be 2")
    if str(np.asarray(data["token_store_checkpoint_sha256"]).item()) != token_store.checkpoint_sha256:
        raise ValueError("Trajectory database and token database use different FSQ tokenizer checkpoints")
    for key, expected in (
        ("representation_family", token_store.representation_family),
        ("representation_variant", token_store.representation_variant),
        ("representation_id", token_store.representation_id),
        ("feature_schema_hash", token_store.feature_schema_hash),
        ("frame_rate", token_store.frame_rate),
        ("temporal_downsample", token_store.temporal_downsample),
        ("receptive_field", token_store.receptive_field),
        ("context_left", token_store.context_left),
        ("lookahead_frames", token_store.lookahead_frames),
        ("decoder_passes_inference", token_store.decoder_passes_inference),
    ):
        if str(np.asarray(data[key]).item()) != str(expected):
            raise ValueError(f"Trajectory database metadata mismatch at {key!r}")
...
def fit_trajectory_normalization(
    ...
    train_windows: Iterable[TokenWindow],
    min_std: float = 1e-6,
...
class ConditionalTokenDataset(Dataset):
    ...
    def __init__(
        ...
        self.tokens = TokenDataset(split, token_store)
        self.trajectory_store = trajectory_store
        self.normalization = normalization

    @property
    def windows(self) -> list[TokenWindow]:
