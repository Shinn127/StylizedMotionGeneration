...
def from_xform(ts):
    return normalize(
        np.where(
            (ts[..., 2, 2] < 0.0)[..., np.newaxis],
            np.where(
                (ts[..., 0, 0] > ts[..., 1, 1])[..., np.newaxis],
                np.concatenate(
                    [
...
def from_xform_xy(x):
    ...
    return from_xform(np.concatenate([c0[..., np.newaxis], c1[..., np.newaxis], c2[..., np.newaxis]], axis=-1))
...
def to_euler(x, order="xyz"):
    ...
    if order == "xyz":
        ...
        raise NotImplementedError(f"Cannot convert from ordering {order}")
