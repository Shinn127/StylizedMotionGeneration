...
class CausalMotionVQVAE(nn.Module):
    def __init__(
        ...
        self.pose_dim = motion_dim - root_cond_dim
        self.upsample_factor = stride_t ** down_t

        encoder_input_dim = self.pose_dim if use_root_cond else motion_dim
    ...
    def _split_features(self, x):
        if not self.use_root_cond:
            return x, None
        return x[..., self.root_cond_dim :], x[..., : self.root_cond_dim]
    ...
    def forward(self, x):
        x_enc, root_cond = self._split_features(x)
        if self.use_root_cond:
            self._validate_root_cond(root_cond, x.shape[0], x.shape[1])
        z_e = self.encoder(self._encode_input(x_enc))
    ...
    def encode_to_indices(self, x):
        x_enc, _ = self._split_features(x)
        z_e = self.encoder(self._encode_input(x_enc))
